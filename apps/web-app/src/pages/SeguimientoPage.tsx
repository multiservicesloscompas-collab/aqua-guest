...
export function SeguimientoPage() {
  ...
  const { noPagados, agendados, enviados } = useMemo(() => {
    const noPagados = rentals.filter(r => !r.isPaid && r.status !== 'finalizado');
  ...
  const RentalCard = ({ rental, showPaymentStatus = false }: { rental: WasherRental; showPaymentStatus?: boolean }) => (
    <Card className="bg-accent/30 border-border/50">
      <CardContent className="p-4 space-y-3">
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-2">
            <WashingMachine className="w-4 h-4 text-primary" />
            <span className="font-medium text-sm">{getMachineName(rental.machineId)}</span>
          </div>
          <div className="flex gap-1">
            <Badge variant={rental.status === 'agendado' ? 'secondary' : 'default'} className="text-xs">
        ...
        <div className="flex items-center justify-between pt-2 border-t border-border/50">
          <div className="flex items-center gap-4 text-xs text-muted-foreground">
            <div className="flex items-center gap-1">
              ...
              <span>{rental.deliveryTime} → {rental.pickupTime}</span>
            </div>
            <span>{RentalShiftConfig[rental.shift].label}</span>
          </div>
          <div className="flex items-center gap-1 text-sm font-semibold text-primary">
            <DollarSign className="w-3.5 h-3.5" />
            <span>{rental.totalUsd.toFixed(2)}</span>
          </div>
        </div>

        <div className="text-xs text-muted-foreground">
          Fecha: {new Date(rental.date).toLocaleDateString('es-VE', { weekday: 'short', day: 'numeric', month: 'short' })}
          {rental.pickupDate !== rental.date && (
            <span className="ml-2">
              → Recogida: {new Date(rental.pickupDate).toLocaleDateString('es-VE', { weekday: 'short', day: 'numeric', month: 'short' })}
            </span>
          )}
        </div>
      </CardContent>
