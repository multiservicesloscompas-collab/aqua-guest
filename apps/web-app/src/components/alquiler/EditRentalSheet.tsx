...
import { WashingMachine, Truck, Clock, MapPin, DollarSign, Calendar, Pencil } from 'lucide-react';
import { WasherRental, RentalShift, RentalShiftConfig, RentalStatus, RentalStatusLabels } from '@/types';
...
export function EditRentalSheet({ rental, open, onOpenChange }: EditRentalSheetProps) {
  const { updateRental, washingMachines, rentals } = useAppStore();
  ...
  useEffect(() => {
    if (rental) {
      ...
      setDeliveryTime(rental.deliveryTime);
  ...
  const unavailableMachines = useMemo(() => {
    if (!rental) return [];
    return rentals
      .filter(r => r.date === rental.date && r.status !== 'finalizado' && r.id !== rental.id)
      .map(r => r.machineId);
  }, [rentals, rental]);
        ...
        <div className="overflow-y-auto h-[calc(100%-8rem)] space-y-6 pb-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-2">
              ...
              <Select value={isPaid ? 'paid' : 'pending'} onValueChange={(v) => setIsPaid(v === 'paid')}>
                <SelectTrigger className="h-12">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
            ...
            <div className="bg-accent/50 rounded-lg p-3 flex items-center gap-2">
              ...
              <div className="text-sm">
                ...
                <span className="font-medium">
                  {formatPickupInfo(pickupInfo.pickupDate, pickupInfo.pickupTime, rental.date)}
                </span>
              </div>
            </div>
          </div>
