...
- **Fuente canónica de propina en visualización:** en cards y breakdowns de ventas editadas, si existe un registro enlazado en `tips` para el `originId` de la venta, ese monto/método de captura debe prevalecer sobre cualquier inferencia `totalBs - sum(items)`. La resta contra `items` queda solo como fallback para registros legacy sin tip enlazada. Cuando existe tip enlazada, el subtotal visible debe derivarse de `total final - propina` para que el desglose mostrado cierre con el total de la card aunque los `items` sigan con subtotal histórico.
...
- **Guard rail de edición simple:** cuando una venta no tiene `paymentSplits` mixtos válidos, el formulario de edición debe mantener `isMixedPayment = false` y dejar vacío el input del monto secundario; el toggle nunca debe inferirse por un string no vacío en ese campo.
