...
- **Fuente canónica de propina en visualización:** en cards y breakdowns de alquileres, si existe un registro enlazado en `tips` para el `originId` del alquiler, ese monto de propina prevalece sobre cualquier inferencia derivada del total. El subtotal visible debe derivarse de `total final - propina` para que la suma mostrada cierre matemáticamente con el total de la card.
