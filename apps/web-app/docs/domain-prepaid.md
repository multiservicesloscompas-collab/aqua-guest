...
7.  **Offline Queue Contract:** When the app is offline, `addPrepaidOrder`, `updatePrepaidOrder`, `deletePrepaidOrder`, and `markPrepaidAsDelivered` must enqueue mutations into the global sync queue (`prepaid_orders` table actions) and update local Zustand state optimistically so operations replay on reconnect.
