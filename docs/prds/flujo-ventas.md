...
**Ubicación**: `apps/web-app/src/pages/WaterSalesPage.tsx`
...
if (liters < product.minLiters || liters > product.maxLiters) {
  toast.error(
    `Los litros deben estar entre ${product.minLiters} y ${product.maxLiters}`
  );
  return;
}
```
...
describe('calculatePricePerLiter', () => {
  it('should return correct price for 25 liters', () => {
    const pricing = [
      { breakpoint: 0, price: 1.5 },
      { breakpoint: 20, price: 1.3 },
      { breakpoint: 50, price: 1.0 },
    ];
    expect(calculatePricePerLiter(25, pricing)).toBe(1.3);
  });
});
```
